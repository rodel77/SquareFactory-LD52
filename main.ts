...
let like_image;
let font, font2;
let pick: p5.SoundFile, pling1, pling3, checkmate;
...
function draw_house(x: number, y: number){

}

function draw_resource(x, y, resource, color?: any){
...
const size = .70;
function draw_hex(x: number, y: number){
    push()
    // scale(size, size)
    // colorMode(HSB);
...
function draw(){
    ...
    if(input_down!=undefined && millis()-input_down>500){
        ...
        if(build_update>map(min(10_000, max(5000, millis()-input_down)), 5000, 10000, 100, 10)){
            build_update = 0;
            buy_factory();
        }
    }
...
function buy_factory(){
    if(init_state && focused_factory!="CIRCLE") return false;
    if(!init_state){
